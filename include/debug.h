...
/**
 * This method can be used to fill a buffer with a sine wave for debugging
 * purposes.
 *
...
//// SERIAL LOGGING
/**
 * C-macro based logging library by Thomas M. To use, simply include this header
 * and call:
 *  - dbg_log(format, args...)
 ...
 */
    ...
    (defined(DEBUG_FATAL) || defined(DEBUG_WARNING) || defined(DEBUG_INFO) || \
     defined(DEBUG_VERBOSE))
/**
 * Log a fatal error to the serial port. Appends a new line at the end of the
 * message.
 *
 ...
 */
#define dbg_log_fatal(format, ...) \
    Serial.printf("[FATAL] " format "\n", ##__VA_ARGS__)
#else
#define dbg_log_fatal
#endif

#if defined(DEBUG_SERIAL_LOGGING) && \
    (defined(DEBUG_WARNING) || defined(DEBUG_INFO) || defined(DEBUG_VERBOSE))
/**
 * Log a warning to the serial port. Appends a new line at the end of the
 * message.
 *
 * @param format the string format to print (format must be a string)
 * @param ... any additional parameters to be formatted into the string
...
#define dbg_log_warning(format, ...) \
    Serial.printf("[WARNING] " format "\n", ##__VA_ARGS__)
#else
#define dbg_log_warning
#endif

#if defined(DEBUG_SERIAL_LOGGING) && \
    (defined(DEBUG_INFO) || defined(DEBUG_VERBOSE))
/**
 * Log a message to the serial port. Appends a new line at the end of the
 * message.
 *
 * @param format the string format to print (format must be a string)
 * @param ... any additional parameters to be formatted into the string
...
/**
 * Log a debugging message to the serial port. *Doesn't* append a new line at
