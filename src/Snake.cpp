...
enum MoveDir : uint8_t { Up = 0, Down = 2, Left = 3, Right = 1 };
...
// ===================================
// LOCALS
#define SNAKE_CELL_SIZE 1
#define SNAKE_CELL_COUNT \
    (DISPLAY_WIDTH / SNAKE_CELL_SIZE * DISPLAY_HEIGHT / SNAKE_CELL_SIZE)
...
void initSnake() {
    ...
    for (int i = 0; i < SNAKE_CELL_COUNT; i++) snake_grid[i] = CellType::Empty;
...
void drawSnake() {
    ...
    if (t - last_move_time >= move_rate) {
        ...
        Vec2 head;
        // These are needed to offset the rectangle drawing so
        // that the segments of the snake connect correctly
        Vec2 draw_offset;
        Vec2 draw_size;
        switch (move_dir) {
            case MoveDir::Up:
                head = {last_head.x, (int16_t)(last_head.y - 1)};
                if (head.y < 0) head.y = SNAKE_GRID_HEIGHT - 1;
                draw_offset = {0, 0};
            ...
            case MoveDir::Left:
                ...
                if (head.x < 0) head.x = SNAKE_GRID_WIDTH - 1;
                draw_offset = {0, 0};
        ...
        display.drawPixel(tail.x, tail.y);
        // display.fillRect(tail.x * SNAKE_CELL_SIZE, tail.y * SNAKE_CELL_SIZE,
        //                  SNAKE_CELL_SIZE, SNAKE_CELL_SIZE, 0);
        ...
        // Advance head
        snake_head++;
        if (snake_head >= SNAKE_CELL_COUNT) snake_head = 0;
        snake_cells[snake_head] = head;
        snake_grid[head.x + head.y * SNAKE_GRID_WIDTH] = CellType::Snake;
        display.setDrawColor(1);
        display.drawPixel(head.x, head.y);
        // display.fillRect(head.x * SNAKE_CELL_SIZE + draw_offset.x,
        //                  head.y * SNAKE_CELL_SIZE + draw_offset.y,
        //                  draw_size.x, draw_size.y, 1);

        // Wrap circular buffer
        if (snake_tail >= SNAKE_CELL_COUNT) snake_tail = 0;

        drawFood();
    }

    move_dir = (MoveDir)(((int8_t)move_dir +
                          max((int8_t)-1, min(encoder_dir, (int8_t)1))) &
                         3);
    encoder_dir = 0;
...
void drawFood() {
    display.drawPixel(food_pos.x, food_pos.y);
    /*display.drawFastHLine(food_pos.x * SNAKE_CELL_SIZE + 1, food_pos.y *
    SNAKE_CELL_SIZE, SNAKE_CELL_SIZE - 2, 1); display.drawFastHLine(food_pos.x *
    SNAKE_CELL_SIZE + 1, food_pos.y * SNAKE_CELL_SIZE + SNAKE_CELL_SIZE - 1,
    SNAKE_CELL_SIZE - 2, 1); display.drawFastVLine(food_pos.x * SNAKE_CELL_SIZE,
    food_pos.y * SNAKE_CELL_SIZE + 1, SNAKE_CELL_SIZE - 2, 1);
    display.drawFastVLine(food_pos.x * SNAKE_CELL_SIZE + SNAKE_CELL_SIZE - 1,
    food_pos.y * SNAKE_CELL_SIZE + 1, SNAKE_CELL_SIZE - 2, 1);*/
}
